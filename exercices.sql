...
select c.nom_categ, c.nb_points from categorie c where c.nb_points = 3; 

--3. Liste des villages (noms) contenant plus de 35 huttes. (4 lignes)
select v.nom_village , v.nb_huttes from village v where v.nb_huttes >35;
...
select h.nom from habitant h where nom like 'A%' and nom like '%r%';

--6. Numéros des habitants ayant bu les potions numéros 1, 3 ou 4. (8 lignes)
select distinct  a.num_hab from absorber a  where num_potion  in  (1,3, 4);
...
and a.date_a between '2052-02-01'and '2052-02-27'; 
...
select r.superficie, v.nom_village , r.nom_resserre  from resserre r 
...
select avg(h.age), p.nom_province  from habitant h 
join village v  on h.num_village = v.num_village 
...
where exists (
select r.num_resserre  from resserre r
...
--3Nom0. s des habitants ayant pris plus de trophées qu'Obélix (3 lignes).

--select h.nom, sum(t.num_trophee)  from habitant h
--join trophee t on h.num_hab = t.num_preneur 
--group by habitant
--having sum(t.num_trophee)> h.num_hab  ='Obélix';

--select h.nom from habitant h 
--join trophee t on h.num_hab = t.num_preneur 
--where t.num_trophee = ( select sum(t2.num_trophee)from trophee t2 
--having t2.num_trophee > 'Obélix');

select h.nom  from habitant h
join trophee t  on t.num_preneur = h.num_hab 
group by h.nom
having  count(t.num_preneur)> (select count(t2.num_preneur) from trophee t2
